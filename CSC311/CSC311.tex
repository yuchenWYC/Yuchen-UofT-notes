...
\begin{align}
	...
	&= \frac{\partial}{\partial \theta} \left [p(S_0)\prod_{t=0}^T\pi_\theta(A_t | S_t) \prod_{t=1}^Tp(S_t|S_{t-1},A_{t-1})\right] &&\text{(by \ref{p of tau})}\\
	&=\frac{\partial}{\partial \theta} \prod_{t=0}^T\pi_\theta(A_t | S_t) &&\text{($\theta$ doesn't depend on $p$)}\\
