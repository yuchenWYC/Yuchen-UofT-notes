...
In a winged-edge mesh, each edge stores pointers to the two vertices it con- nects (the head and tail vertices), the two faces it is part of (the left and right faces), and, most importantly, the next and previous edges in the counterclock- wise traversal of its left and right faces (Figure 12.16). Each vertex and face also stores a pointer to a \under{single, arbitrary edge} that connects to it:
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4]{p21}
\end{figure}
