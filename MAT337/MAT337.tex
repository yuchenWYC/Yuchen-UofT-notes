...
%\example
%Given $(X, d)$ a metric space, define $\tau_d: U \in \tau_d \iff \forall x \in U, \exists \epsilon > 0, B_\epsilon(x) \subseteq U$. Then $\tau_d$ is a topology.

\definition
$(X, \tau)$ is a discrete topological space iff $\tau = P(X)$.

\definition
A subset $F$ of a topological space $(X, \tau)$ is closed if $X \setminus F$ is open. \\
\tb{Properties: }
\begin{enumerate}
