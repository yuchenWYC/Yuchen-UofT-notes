...
\begin{document}
    ...
    Let $S$ be any contiguous subset of $\mb{Z}$.\\
    ...
    \begin{proof}
    	\begin{align*}
    		...
    		\implies \pi_i p_{ij} &= \pi_j p_{ji} \text{ if $j = i+1$, hence for all $i, j \in S$}
    	\end{align*}
   ...
   \remark This says that, with probability 1 the Gambler's Ruin game must eventually end, and the time it takes to end has finite expected value.
    ...
    Since $\dur{Y}{0}{M} = 0$ for all $M$, also $B_0 = 0$. Also, note that $\dur{Y}{t}{M} = \frac{1}{\sqrt{M}}(Z_1 + Z_2 + \hdots + Z_{tM})$ (at least if $tM \in \mb{Z}$, otherwise within $\mc{O}(1/\sqrt{M})$, which )    
